...
int CondInit(Cond *c) {
  ...
  if (AQueueInit (&c->waiting) != QUEUE_SUCCESS) {
    printf("FATAL ERROR: could not initialize condition waiting queue in CondInit!\n");
    exitsim();
  }
  return SYNC_SUCCESS;
}
...
int CondWait(Cond *c) {
  ...
  //get the lock
  // Lock lock = locks[c->lock];

  //first release lock
  if (LockHandleRelease(c->lock) != SYNC_SUCCESS) {
    printf("FATAL ERROR: could not acquire lock inside the condition variable!\n");
    exitsim();  
  }

  //put currentPCB into sleep
  if ((l = AQueueAllocLink ((void *)currentPCB)) == NULL) {
    printf("FATAL ERROR: could not allocate link for lock queue in LockAcquire!\n");
    exitsim();
  }
  ...
  if (LockHandleAcquire(c->lock) != SYNC_SUCCESS) {
    printf("FATAL ERROR: lock acquire fail at CondWait\n");
    exitsim();
  }
  RestoreIntrs(intrval);
  return SYNC_SUCCESS;
...
int CondSignal(Cond *c) {
  ...
  intrs = DisableIntrs ();
  dbprintf ('s', "CondSignal: Process %d Signalling on condition %d, lock=%d.\n", GetCurrentPid(), (int)(c-conds), c->lock);
  // Increment internal counter before checking value
...
int CondBroadcast(Cond *c) {
  ...
  qlength = AQueueLength(&c->waiting);
