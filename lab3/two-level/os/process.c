...
void ProcessFreeResources (PCB *pcb) {
  ...
  for (i = 0; i < MEM_L1TABLE_SIZE; i++) {
      ...
      if (ptr != NULL) {
        for (j = 0; j < MEM_L2TABLE_SIZE; j++) {
          MemoryFreePage(ptr[j] / MEM_PAGESIZE);
          ptr[j] = 0;
        }
      }
      FreeL2Page(pcb->l1_table[i]);
      pcb->l1_table[i] = NULL;
  }
  ...
  //   MemoryFreePage((pcb->l1_table)[i] / MEM_PAGESIZE);
  // }
...
int ProcessFork (VoidFunc func, uint32 param, char *name, int isUser) {
  ...
  pcb->l1_table[0] = AllocL2Page();
  if (pcb->l1_table[0] == -1) {
    printf("FATAL ERROR: ProcessFork could not allocate more L2 page for user code and global data\n");
    exitsim();   
  }
  ptr = pcb->l1_table[0];
  for (i = 0; i < PROCESS_CODEDATA; i++) {
    ...
    ptr[i] = MemorySetupPte(physical_page_idx);
    dbprintf('m', "Student - ProcessFork: ptr[%d] = %d\n", i, ptr[i]);
  }
  ...
  pcb->l1_table[MEM_L1TABLE_SIZE-1] = AllocL2Page();
  if (pcb->l1_table[MEM_L1TABLE_SIZE-1] == -1) {
    printf("FATAL ERROR: ProcessFork could not allocate more L2 page for user stack\n");
    exitsim();     
  }
  ptr = pcb->l1_table[MEM_L1TABLE_SIZE-1];
  ptr[MEM_L2TABLE_SIZE-1] = MemorySetupPte(physical_page_idx);
  dbprintf('m', "Hello, here\n");
  dbprintf('m', "Usr stack pte: %d\n", ptr[MEM_L2TABLE_SIZE-1]);

  // init system stack, and I think for two level paging, the system stack related variables is unchanged
  dbprintf('m', "Student - ProcessFork: start to allocate 1 page for system stack. Yeah\n");
  physical_page_idx = MemoryAllocPage();
  if (physical_page_idx == -1) {
  ...
  stackframe[PROCESS_STACK_PTBASE] = (uint32)&(pcb->l1_table[0]);
