...
void ProcessModuleInit () {
  ...
  for (i = 0; i < PROCESS_MAX_PROCS; i++) {
    ...
    //-------------------------------------------------------
    //@Michael: Could we do lazy init?
    dbprintf ('m', "Student - ProcessModuleInit: start to initialize the PCB's page table\n");
    for (j = 0; j < MEM_L1TABLE_SIZE; j = j + 1){
...
void ProcessFreeResources (PCB *pcb) {
  ...
  //------------------------------------------------------------
  dbprintf ('m', "Student - ProcessFreeResources: start to free any memory resources on process death\n");

  dbprintf ('m', "Student - ProcessFreeResources: start to free system stack\n");
  MemoryFreePage((pcb->sysStackArea) / MEM_PAGESIZE);
  pcb->sysStackArea = 0;
  dbprintf ('m', "Student - ProcessFork: start to free user stack\n");
  //@Michael: MemoryFreePage need to support free null pages (not efficient solution, but works)
  for (i = 0; i < MEM_L1TABLE_SIZE; i++) {
      MemoryFreePage((pcb->pagetable[i] ) / MEM_PAGESIZE);
      pcb->pagetable[i] = 0;
  }
  //@Michael: Actually this is unnecessary
  dbprintf ('m', "Student - ProcessFork: start to free user code and global data\n");
  for (i = 0; i < PROCESS_CODEDATA; i++){
    MemoryFreePage((pcb->pagetable)[i] / MEM_PAGESIZE);
  }
...
int ProcessFork (VoidFunc func, uint32 param, char *name, int isUser) {
  ...
  // User stack
  // Assign 4 pages
  dbprintf ('m', "Student - ProcessFork: start to allocate pages for a new process\n");

  // init 4 pages for user code and global data
  dbprintf('m', "Student - ProcessFork: start to allocate 4 pages for user code and global data\n");
  for (i = 0; i < PROCESS_CODEDATA; i++) {
    physical_page_idx = MemoryAllocPage();
    if (physical_page_idx == -1) {
      printf("FATAL ERROR: ProcessFork could not allocate more physical page for user code and global data\n");
      exitsim();
    }
    pcb->pagetable[i] = MemorySetupPte(physical_page_idx);
    dbprintf('m', "Student - ProcessFork: pcb->pagetable[%d] = %d\n", i, pcb->pagetable[i]);
  }

  // init user stack
  dbprintf('m', "Student - ProcessFork: start to allocate 1 page for user stack\n");
  physical_page_idx = MemoryAllocPage();
  if (physical_page_idx == -1) {
    printf("FATAL ERROR: ProcessFork could not allocate more physical page for user stack\n");
    exitsim();
  }
  pcb->pagetable[MEM_L1TABLE_SIZE-1] = MemorySetupPte(physical_page_idx);

  // init system stack
  dbprintf('m', "Student - ProcessFork: start to allocate 1 page for system stack\n");
  physical_page_idx = MemoryAllocPage();
  if (physical_page_idx == -1) {
    printf("FATAL ERROR: ProcessFork could not allocate more physical page for system stack\n");
    exitsim();
  }
  stackframe = ((uint32 *)((physical_page_idx + 1) * MEM_PAGESIZE - 4));
  pcb->sysStackArea = physical_page_idx * MEM_PAGESIZE;
  ...
  if (isUser) {
    ...
    stackframe[PROCESS_STACK_USER_STACKPOINTER] = (((uint32)MEM_MAX_VIRTUAL_ADDRESS) & (~0x3));
    dbprintf('m', "stackframe[PROCESS_STACK_USER_STACKPOINTER] = %x\n", stackframe[PROCESS_STACK_USER_STACKPOINTER]);
...
//  ProcessPTE
//
//  Print the PTE of a process
//----------------------------------------------------------------------
void ProcessPrintPTE (int pid){
  int i;                   // Loop index variable
  PCB currentpcb;

  currentpcb = pcbs[pid];
  // Print PTE for 4 pages of user code and global data
  printf('m', "PTE for user code and global data\n");
  for (i = 0; i < MEM_L1TABLE_SIZE; i++) {
    //Increment the reference bit by 1
    //printf("Current PTE%d of process%d is %d\n", i, pid, currentpcb.pagetable[i]);
    dbprintf('m',"Current PTE%d of process%d is %d\n", i, pid, currentpcb.pagetable[i]);
  }
  return;
}
...
//  ProcessRealFork
//
//  The actual function to fork a process.  This involves the following steps:
//  * Allocate resources for the process (PCB, memory, etc.)
//  * Initialize the resources by copying information from parent process to child process
//  * Place the PCB on the runnable queue
//----------------------------------------------------------------------
int ProcessRealFork (PCB *parentpcb) {
  int i;                   // Loop index variable
  uint32 *stackframe;      // Stores address of current stack frame.
  ...
  intrs = DisableIntrs ();
  dbprintf ('I', "Old interrupt value was 0x%x.\n", intrs);
  dbprintf ('m', "Entering ProcessRealFork PID = %p\n", currentPCB);
  // Get a free PCB for the new child process
  if (AQueueEmpty(&freepcbs)) {
    printf ("FATAL error: no free processes!\n");
    exitsim (); // NEVER RETURNS!
  }
  ...
  // At this point, the new child PCB is allocated and nobody else can get it.
  // However, it's not in the run queue, so it won't be run.  Thus, we
  // can turn on interrupts here.
  RestoreIntrs (intrs);

  // Copy the process name into the child PCB.
  dstrcpy(childpcb->name, parentpcb->name);

  //----------------------------------------------------------------------
  // This section initializes the memory for this process
  //----------------------------------------------------------------------
  // Allocate 1 page for system stack
  // Share 1 page for user stack (at top of virtual address space), and 4 pages for user code and global data.
  //---------------------------------------------------------
  dbprintf ('m', "Student - ProcessRealFork: Start to share or allocate pages for a new child process\n");

  // Duplicate the PCB
  bcopy((char *)parentpcb, (char *)childpcb, sizeof(PCB));  

  // Share the 4 pages for user code and global data to child process, don't need to allocate new pages here
  dbprintf('m', "Student - ProcessRealFork: Share the 4 pages for user code and global data\n");
  for (i = 0; i < PROCESS_CODEDATA; i++) {
    //Set up the read only bit
    parentpcb->pagetable[i] |= MEM_PTE_READONLY;
    childpcb->pagetable[i] |= MEM_PTE_READONLY;
    //Increment the reference bit by 1
    MemorySharePage((parentpcb->pagetable[i])/MEM_PAGESIZE);
  }

  // Share user stack to child process
  dbprintf('m', "Student - ProcessRealFork: Share the 1 page for user stack\n");
  parentpcb->pagetable[MEM_L1TABLE_SIZE-1] |= MEM_PTE_READONLY;
  childpcb->pagetable[MEM_L1TABLE_SIZE-1] |= MEM_PTE_READONLY;
  MemorySharePage((parentpcb->pagetable[MEM_L1TABLE_SIZE-1])/MEM_PAGESIZE);

  // For system stack, we will need to allocate a page and copy from parent process to child process
  dbprintf('m', "Student - ProcessRealFork: Allocate 1 page for child process' system stack and copy the content from parent process\n");
  physical_page_idx = MemoryAllocPage();
  if (physical_page_idx == -1) {
    printf("FATAL ERROR: ProcessRealFork could not allocate more physical page for system stack\n");
    exitsim();
  }
  stackframe = ((uint32 *)((physical_page_idx + 1) * MEM_PAGESIZE - 4));
  childpcb->sysStackArea = physical_page_idx * MEM_PAGESIZE;
  bcopy((char *)(parentpcb->sysStackArea), (char *)(childpcb->sysStackArea), MEM_PAGESIZE);
  dbprintf('m', "Student - Child process: Reference counter of system stack page\n");
  MemoryGetPageRC(physical_page_idx);
  dbprintf('m', "Student - Parent process: Reference counter of system stack page\n");
  MemoryGetPageRC(parentpcb->sysStackArea / MEM_PAGESIZE);
  ...
  // The previous stack frame pointer is set to 0, meaning there is no
  // previous frame.
  dbprintf('m', "ProcessRealFork: stackframe = 0x%x\n", (int)stackframe);
  stackframe[PROCESS_STACK_PREV_FRAME] = 0;
  ...
  // stack frame.
  //----------------------------------------------------------------------
  stackframe[PROCESS_STACK_PTBASE] = (uint32)&(childpcb->pagetable[0]);
  stackframe[PROCESS_STACK_PTSIZE] = MEM_L1TABLE_SIZE;
  stackframe[PROCESS_STACK_PTBITS] = (MEM_L1FIELD_FIRST_BITNUM << 16) | (MEM_L1FIELD_FIRST_BITNUM);
  ...
  //---------------------------------------------------------------------
  //The return value of fork() is set to 0 in the child process, and 
  //it is set to the processid of the new child in the parent process.
  //Use ProcessSetResult() to return values
  //---------------------------------------------------------------------
  ProcessSetResult(childpcb, 0);
  ProcessSetResult(parentpcb, GetPidFromAddress(childpcb));

  //Print the PTE of Parent and Child process
  //Code and Global data
  for (i = 0; i < PROCESS_CODEDATA; i++) {
    dbprintf('m',"Parent process%d: Code and Global data page%d PTE of is %d\n", GetPidFromAddress(parentpcb), i, parentpcb->pagetable[i]);
    dbprintf('m',"Child process%d: Code and Global data page%d PTE of is %d\n", GetPidFromAddress(childpcb), i, childpcb->pagetable[i]);
  }
  //User stack
  dbprintf('m',"Parent process%d: User Stack page PTE of is %d\n", GetPidFromAddress(parentpcb), parentpcb->pagetable[MEM_L1TABLE_SIZE-1]);
  dbprintf('m',"Child process%d: User Stack page PTE of is %d\n", GetPidFromAddress(childpcb), childpcb->pagetable[MEM_L1TABLE_SIZE-1]);
  //System stack
  dbprintf('m',"Parent process%d: System Stack page PTE of is %d\n", GetPidFromAddress(parentpcb), parentpcb->sysStackArea);
  dbprintf('m',"Child process%d: System Stack page PTE of is %d\n", GetPidFromAddress(childpcb), childpcb->sysStackArea);

  dbprintf ('m', "Leaving ProcessRealFork (%s)\n", childpcb->name);
  return (1);
}
