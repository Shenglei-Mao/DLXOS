...
//--------------------------------------------------------------------
uint32 GetUintFromTrapArg(uint32 *trapArgs, int sysmode)
{
  uint32 arg;
  if(!sysmode)
  {
	     ...
	     uint32 *trapArgs)
{
  ...
  if (cause & TRAP_TRAP_INSTR) {
    ...
    case TRAP_PROCESS_FORK:
      ...
      ProcessRealFork(currentPCB);   //PCW
      break;
    ...
    case TRAP_MALLOC:
      ihandle = GetIntFromTrapArg(trapArgs, isr & DLX_STATUS_SYSMODE);
      //ihandle = malloc(currentPCB, ihandle);
      ProcessSetResult(currentPCB, ihandle); //Return handle
      break;
    case TRAP_MFREE:
      ihandle = GetIntFromTrapArg(trapArgs, isr & DLX_STATUS_SYSMODE);
      //ihandle = mfree(currentPCB, ihandle);
      ProcessSetResult(currentPCB, ihandle); //Return handle
      break;
    ...
    case TRAP_ROP_ACCESS:
      printf ("Trap for protection to copy-on-write\n");
